...
MSG=[range(0,2000,1),range(0,2000,1),range(0,2000,1)] 
...
#[53-62] SENSORS3 (imu)
#[63-66] Laser (ang min, ang res, ang max, length of scan)
...
def callback1(scan,rob): #181 data points [63-243]
    global MSG,i
    MSG[rob][63]=scan.angle_min
    MSG[rob][64]=scan.angle_max
    MSG[rob][65]=scan.angle_increment
    MSG[rob][66]=len(scan.ranges)
    MSG[rob][67:67+len(scan.ranges)] = scan.ranges[0:len(scan.ranges)]
    #print MSG[rob][67+len(scan.ranges)-1]
    #print rob,len(scan.ranges),scan.ranges[len(scan.ranges)-1]
...
def callback2(odom,rob): #12 data points [8-19]
    ...
    if rob==0 : 
    	tf_prefix="/pioneer1/base_link"
    if rob==1 : 
    	tf_prefix="/pioneer2/base_link"
    if rob==2 : 
    	tf_prefix="/laser"
    try:
    	now = rospy.Time.now()
       	(trans,rot) = tf_listener.lookupTransform( "/map",tf_prefix, rospy.Time(0))
    	(roll,pitch,yaw) = euler_from_quaternion(rot)
        MSG[rob][8]= trans[0]
     	MSG[rob][9]= trans[1]
    	MSG[rob][10]= trans[2]
...
def listener():
    ...
    rospy.Subscriber('/scan', LaserScan, callback1,2,queue_size=1)
	...
	for rob in [0, 1,2]:
		if rob==0 :
			tf_prefix="rigidbody_1/base_link"
	    	if rob==1 : 
			tf_prefix="rigidbody_2/base_link"
    		if rob==2 : 
			tf_prefix="/laser"
        	try:
			now = rospy.Time.now()
       			(trans,rot) = tf_listener.lookupTransform('/odom',tf_prefix, rospy.Time(0))
			(roll,pitch,yaw) = euler_from_quaternion(rot)
			MSG[rob][0]= now.secs
			MSG[rob][1]= now.nsecs
          		MSG[rob][2]= trans[0]
	...
	if i==1000:
		obj_arr = np.zeros((6,), dtype=np.object)
        	obj_arr[0] = 'pioneer1     [0-1]	 TIME		:secs,nsecs, [2-7]   GROUNDTRUTH	:xw,yx,zw,rollw,pitchw,yaw, [8-19]	 ODOM		:x,y,z,roll,pitch,yaw,vx,vy,vz,omegax,omegay,omegaz, [20-26] IRRM1 (US)	:rangeCorr,r,usaoa, RAW:, US1,US2,US3,US4 [27-37] IRRM2 (IR)	:bearCorr,alpha,beta, RAW:, IRx1,IRy1,IRx2,IRy2,IRx3,IRy3,IRx4,IRy4, CUSTOM RAW DATA [38-52]	  SENSORS2 (navdata): mag xyz,rpy, altd-p, altd-us, vx, vy, vz, motor 1 2 3 4 ,[53-62]  SENSORS3 (imu): acc x y z, omg x y z, q0 q1 q2 q3, [63-243] SENSORS1 (laser)  :laser'
		#print MSGR1
		obj_arr[1] = MSGR1
		...
		obj_arr[5] = MSGR3
        	scipy.io.savemat('3robtest_2015_JUL.mat', {'obj_arr':obj_arr})
		sys.exit()
	rate.sleep()
    

if __name__ == '__main__':  
        rospy.init_node('Relocdataprocessing2', anonymous=True)
