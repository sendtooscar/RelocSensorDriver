...
MSG=[range(0,250,1),range(0,250,1),range(0,250,1)] 
...
def listener():
    ...
    rospy.Subscriber('/drone2/ardrone/navdata', Navdata, callback7,2,queue_size=1)
    rospy.Subscriber('/drone2/ardrone/imu', Imu, callback8,2,queue_size=1)

    rospy.Subscriber('/drone3/ardrone/navdata', Navdata, callback7,1,queue_size=1)
    rospy.Subscriber('/drone3/ardrone/imu', Imu, callback8,1,queue_size=1)
	...
	for rob in [1, 2]:
		if rob==0 : 
			tf_prefix="/Robot1/Tag1"
	    	if rob==1 : 
			tf_prefix="/vicon/Chopper1/Segment1"
    		if rob==2 : 
			tf_prefix="/vicon/Chopper2/Segment2" #this is drone 2
        	try:
			now = rospy.Time.now()
       			(trans,rot) = tf_listener.lookupTransform("/world",tf_prefix, rospy.Time(0))
			(roll,pitch,yaw) = euler_from_quaternion(rot)
			MSG[rob][0]= now.secs
			MSG[rob][1]= now.nsecs
        		MSG[rob][2]= trans[0]
	...
	if i==3500:
		...
		obj_arr[5] = MSGR3
        	scipy.io.savemat('vicontest101.mat', {'obj_arr':obj_arr})
		sys.exit()
	rate.sleep()
